...
\begin{proposition}
  ...
  \begin{itemize}
    \item $m^{*}(E)\geqslant 0$.
    \item $m^{*}(E)=a<\infty \Leftrightarrow \forall \varepsilon >0, \exists \{I_k\}, E\subset \bigcup_{k=1}^{\infty}I_k, \sum_{k=1}^{\infty}|I_k|<a+\varepsilon$
  \end{itemize}
\end{proposition}
...
\begin{proposition}
  若$E\subset F$, 则$m^{*}(E)\leqslant m^{*}(F)$.\par
\end{proposition}
...
\begin{theorem}
  设$E\subset \mathbb{R}^n$, $x+E=\{x+y:y\in E\}$, 则$m^{*}(E)=m^{*}(x+E)$.\par
\end{theorem}
...
\begin{theorem}
  设$E_1, \cdots, E_n\subset \mathbb{R}^n$, 则有$m^{*}\left(\bigcup_{k=1}^{\infty}E_k\right)\leqslant \sum_{k=1}^{\infty}m^{*}(E_k)$.\par
\end{theorem}
...
\begin{theorem}
  设$E_1, \cdots, E_n\subset \mathbb{R}^n$, 若$\forall i,j, \rho(E_i,E_j)>0$, 则有$m^{*}\left(\bigcup_{k=1}^{\infty}E_k\right)= \sum_{k=1}^{\infty}m^{*}(E_k)$.\par
\end{theorem}
...
\begin{definition}
  设$E\subset \mathbb{R}^n$, 则$E$是可测集当且仅当$E$满足Carathéodory条件. 若$E$是可测的, 则定义其测度为$m(E)=m^{*}(E)$.\par
...
\begin{definition}
  设$f$是定义在$E$上的可测函数, 则称$f^{+}(x)=\max\{f(x),0\}$为$f$的正部函数, $f^{-}(x)=\max\{-f(x),0\}$为$f$的负部函数.\par
  于是$f$在$E$上的Lebesgue积分定义为$\int_{E}f(x)\mathrm{d}x=\int_{E}f^{+}(x)\mathrm{d}x-\int_{E}f^{-}(x)\mathrm{d}x$.\par
\end{definition}
...
\begin{theorem}
  (Lebesgue 控制收敛) 设$f_n$几乎处处有限且可测, $F(x)\geqslant |f_n(x)|$, $f_n\Rightarrow f$在$E$上, 则$f\in L(E)$, 且$\int_{E}f(x)\mathrm{d}x= \int_{E}\lim_{n\to \infty}f_n(x)\mathrm{d}x$.\par
\end{theorem}
