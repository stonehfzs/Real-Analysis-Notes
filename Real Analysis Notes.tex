...
\begin{remark}
  Lebesgue控制收敛定理中依测度收敛可以改成其他条件较弱的收敛, 因为前面的若干定理已经证明了全空间测度有限的情况下有几乎处处收敛函数列必定依测度收敛和近乎一致收敛.\par
\end{remark}

类似地, 我们还有Lebesgue有界收敛定理, 可以表示如下:\par

\begin{corollary}
  (Lebesgue 有界收敛) 设$f_n$几乎处处有限且可测, $\exists K >0, K\geqslant |f_n(x)|$, $f_n\Rightarrow f$在$E$上, 则$f\in L(E)$, 且$\int_{E}f(x)\mathrm{d}x=\lim_{n\to \infty} \int_{E}f_n(x)\mathrm{d}x$.\par
\end{corollary}
